...
public class Ch3Ex2Activity extends AppCompatActivity {
    ...
    private void resetTargetAnimation() {
        ...
        ObjectAnimator tsAnimator = ObjectAnimator.ofFloat(target,
                "alpha",1f,0.5f);
        tsAnimator.setRepeatCount(ValueAnimator.INFINITE);
        tsAnimator.setInterpolator(new LinearInterpolator());
        tsAnimator.setDuration(Integer.parseInt(durationSelector.getText().toString()));
        tsAnimator.setRepeatMode(ValueAnimator.REVERSE);
        // TODO ex2-3: 将上面创建的其他 ObjectAnimator 都添加到 AnimatorSet 中
        animatorSet = new AnimatorSet();
        animatorSet.playTogether(animator1,scaleXAnimator,scaleYAnimator,tsAnimator);
        animatorSet.start();
    }
}
